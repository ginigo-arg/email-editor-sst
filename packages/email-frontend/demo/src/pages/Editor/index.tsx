...
function defaultOnChange(val) {
  localStorage.setItem('template', val.fields.content);
}

async function defaultOnImageUpload(val) {
...
export default function Editor(props: Props) {
  const {
    ...
    sendTestEmail,
    onUploadImage = defaultOnImageUpload,
  } = props;

  iv.language = locale.startsWith('es') ? 'spanish' : 'english';
  ...
  const [initialValues] = useState(template);
    ...
    <ConfigProvider locale={enUS}>
      <div className='editor-container'>
        ...
        <EmailEditorProvider
          ...
          socialIcons={[]}
          locale={localesData[locale.split('-')[0]]}
          mediaLibraryRef={mediaLibraryRef}
          enabledLogic={false}
        >
          {({ values }, { submit, restart, getState }) => {
            return (
              <>
                <StandardLayout
                        ...
                        <Button
                          onClick={() => {
                            restart({
                              ...
                              subTitle: 'Nice to meet you!',
                              language: 'english',
                              content: BlockManager.getBlockByType(
                                BasicType.PAGE,
                              )!.create({
                                data: {
                                  value: { fonts: undefined },
                        ...
                        <Dropdown
                          ...
                          droplist={
                            <Menu>
                              <Menu.Item
                                key='Export MJML'
                                onClick={() => onExportMJML(getState().values)}
                              >
                                Export MJML
                              </Menu.Item>
                              <Menu.Item
                                key='Export HTML'
                                onClick={() => onExportHTML(getState().values)}
                              >
                                Export HTML
                              </Menu.Item>
                              {/* <Menu.Item
                              ...
                              <Menu.Item
                                key='Export Image'
                                onClick={() => onExportImage(getState().values)}
                              >
                                Export Image
                              </Menu.Item>
                            </Menu>
